...
// Type: Microsoft.Xna.Framework.GamerServices.PropertyDictionary
    ...
    IEnumerable
  {
    ...
    internal PropertyValue GetProperty(string key, bool demandCreate)
    {
      ...
      if (this.properties.TryGetValue(key, out propertyValue))
        return propertyValue;
      throw new ArgumentException(string.Format((IFormatProvider) CultureInfo.CurrentCulture, FrameworkResources.XlastUnknown, (object) FrameworkResources.XlastProperty, (object) key));
    }
    ...
    private void SetUntypedValue(string key, object value)
    {
      ...
      if (this.isReadOnly && this.fillingReadOnlyData)
        throw new NotSupportedException(string.Format((IFormatProvider) CultureInfo.CurrentCulture, FrameworkResources.ReadOnly, (object) typeof (PropertyDictionary).Name));
    ...
    private void SetTypedValue<T>(string key, T value) where T : struct, IEquatable<T>
    {
      ...
      if (!property.IsChanged || isChanged || this.fillingReadOnlyData)
        return;
      ++this.changedPropertyCount;
      GamerServicesProviderWeb.RequestLeaderboardWrite();
    }
